...
class TranslationsTableSeeder extends Seeder
{
    ...
    {
        // Adding translations for 'categories.blade.php'
        //
        $cat = Category::where('slug', 'category-1')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories.blade.php', 'slug'], $cat->id), 'categoria-1');
            $this->trans('pt', $this->arr(['categories.blade.php', 'name'], $cat->id), 'Categoria 1');
        }
        $cat = Category::where('slug', 'category-2')->firstOrFail();
        if ($cat->exists) {
            $this->trans('pt', $this->arr(['categories.blade.php', 'slug'], $cat->id), 'categoria-2');
            $this->trans('pt', $this->arr(['categories.blade.php', 'name'], $cat->id), 'Categoria 2');
        }
    }
    ...
    private function menusTranslations()
    {
        ...
        $_item = $this->findMenuItem(__('voyager::seeders.menu_items.categories.blade.php'));
        if ($_item->exists) {
